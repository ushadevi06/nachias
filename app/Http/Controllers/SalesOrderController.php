        ...
        if ($request->ajax()) {
            ...
            foreach ($salesOrders as $so) {
                ...
                foreach (['Draft', 'Approved', 'Pending', 'In Production', 'Dispatched', 'Cancelled'] as $status) {
                    ...
                    $disabled = '';
                    if ($so->status === 'Approved' && $status === 'Draft') $disabled = 'disabled';
                    if ($so->status === 'Dispatched' && in_array($status, ['Draft', 'Approved'])) $disabled = 'disabled';
                    if ($so->status === 'Cancelled' && $status !== 'Cancelled') $disabled = 'disabled';
