...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $expiring_documents = DocumentRepository::where('validity_date', '>=', Carbon::now())
            ->where('validity_date', '<=', Carbon::now()->addDays(90))
        ...
        foreach ($jobCards as $jc) {
            ...
            foreach ($stages as $stage) {
                ...
                $stageTasks = $jc->tasks->filter(function ($task) use ($stageId) {
                    if ($task->stage_id == $stageId && !$task->stage) {
                        return true;
                    }
                    if ($task->stage && $task->stage->operation_stage_id == $stageId) {
                        return true;
                    }
                    return false;
                });
                ...
                if ($requiredServices->isEmpty()) {
                    ...
                    $serviceCompletions = $requiredServices->map(function ($service) use ($assignments, $periodInward) {
                        ...
                        $isSequential = $serviceAssignments->count() > 1 && $serviceAssignments->max('issue_qty') >= $periodInward;
                    ...
                    $periodOutward = $serviceCompletions->min();
                }

                if ($periodOutward > $periodInward) {
                    $periodOutward = $periodInward;
                }

                $currentWip = max(0, $periodInward - $periodOutward);

                $isStepCompletelyFinished = ($periodInward > 0 && $periodOutward >= $periodInward) && ($currentWip == 0);
        ...
        $process_wise_status = DB::table('tasks')->join('operation_stages', 'tasks.stage_id', '=', 'operation_stages.id')->whereNull('tasks.deleted_at')->select('operation_stages.operation_stage_name', DB::raw("SUM(CASE WHEN tasks.status = 'Planned' THEN 1 ELSE 0 END) as planned"), DB::raw("SUM(CASE WHEN tasks.status = 'In Progress' THEN 1 ELSE 0 END) as in_progress"), DB::raw("SUM(CASE WHEN tasks.status = 'Completed' THEN 1 ELSE 0 END) as completed"))->groupBy('operation_stages.id', 'operation_stages.operation_stage_name')->get();

        return view('dashboard', compact(
            'sales_today', 'sales_month', 'sales_year', 'sales_count_today', 'sales_count_month', 'sales_count_year',
            'orders_today', 'orders_month', 'total_stock', 'urgent_orders', 'total_sales_value', 'sales_return', 'bill_discount',
