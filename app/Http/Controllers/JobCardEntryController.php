...
class JobCardEntryController extends Controller
{
    ...
    public function add(Request $request, $id = null) {
        ...
        $stageTaskStatus = [];
        if ($jobCard) {
            $tasks = Task::where('job_card_entry_id', $jobCard->id)->get();
            foreach ($tasks as $task) {
                if ($task->stage_id) {
                    $stageTaskStatus[$task->stage_id] = [
                        'status' => $task->status,
        ...
        return view('job_card_entry/add', compact(
            ...
            'brandCategories', 'operationStages', 'stageTaskStatus'
        ));
    }
            ...
            try {
                if ($request->items) {
                    ...
                    foreach ($request->items as $matrixId => $itemData) {
                        ...
                        if ($existingItem && $existingItem->stock_entry_item_id) {
                    ...
                    foreach ($request->items as $matrixId => $itemData) {
                        ...
                        if ($artNo && $qtyIssue > 0) {
                            ...
                            foreach ($stockCandidates as $stockItem) {
                                ...
                                if (!isset($firstStockItemId)) $firstStockItemId = $stockItem->id;
        ...
        if ($artData->isEmpty()) {
            ...
            $artData = $po->items->map(function($item) {
                ...
                if ($item->raw_material_id) {
                    $fallbackStock = \App\Models\StockEntryItem::whereIn('grn_entry_item_id', function($q) use ($grnIds) {
    ...
    public function checkStock($id) {
        ...
        if ($artData->isEmpty()) {
            $artData = $po->items->map(function($item) use ($grnIds) {
                $netStock = 0;
                if ($item->raw_material_id) {
                    $netStock = \App\Models\StockEntryItem::whereIn('grn_entry_item_id', function($q) use ($grnIds) {
                        $q->select('id')->from('grn_entry_items')->whereIn('grn_entry_id', $grnIds);
                    })
                    ->where('raw_material_id', $item->raw_material_id)
                    ->sum(\Illuminate\Support\Facades\DB::raw('qty_in - COALESCE(qty_out, 0)'));
    ...
    private function syncSchedulesFromJobCard($jobCard, $stagesData)
    {
        ...
        foreach ($stagesData as $stage) {
            if (empty($stage['stage_id'])) continue;
    ...
    private function generateProductionConsumables($jobCard)
    {
        ...
        foreach ($stages as $stage) {
            ...
            foreach ($fabricDetails as $fabricDetail) {
                ...
                // If it has site-wise consumptions (Category 1 logic)
            ...
            // 2. Process Issue Items (Other consumables like Buttons, etc.)
