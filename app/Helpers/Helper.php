    ...
    function numberToWords($number)
    {
        ...
        while ($i < $digits_length) {
            ...
            if ($num) {
                ...
                $str[] = ($num < 21) ? $words[$num] . ' ' . $digits[$counter] . $plural . ' ' . $hundred
        ...
        $paise = '';
        if ($decimal > 0) {
            $paiseStr = ($decimal < 21) ? $words[$decimal] : ($words[floor($decimal / 10) * 10] . ' ' . $words[$decimal % 10]);
            $paise = trim($paiseStr) . ' Paise';
        }
        ...
        if (!empty($paise)) {
            $out .= (!empty($out) ? ' and ' : '') . $paise;
        }
        if (empty($out)) {
            $out = 'Zero Rupees';
        }
        return strtoupper(trim($out) . ' ONLY');
    }
}
