...
@section('content')
    ...
    @php
        ...
        foreach ($fabrics as &$fabric) {
            if (isset($fabric['art_no'])) {
                $rm = \App\Models\RawMaterial::where('code', $fabric['art_no'])->orWhere('name', $fabric['art_no'])->first();
                $fabric['store_category_id'] = $rm ? $rm->store_category_id : 1;
            }
        }
        unset($fabric);
...
<script>
    $(document).ready(function() {
        ...
        let currentArtData = (@json(array_values($fabrics)) || []).map(f => ({
            ...
            store_category_id: f.store_category_id,
            stock_entry_id: f.stock_entry_id
        }));
        ...
        (function () {
            ...
            if ($('#allTagsModal').length === 0) {
                $('body').append(`
                <div class="modal fade" id="allTagsModal" tabindex="-1" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-scrollable modal-lg">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title">Selected Materials</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <div class="modal-body" id="allTagsModalBody" style="display: flex; flex-wrap: wrap; gap: 4px;">
                      </div>
                    </div>
                  </div>
                </div>`);
            ...
            function renderAllTags() {
                $tags.empty();
                $('#allTagsModalBody').empty();
                
                const limit = 5;
                const toShow = selectedTagData.slice(0, limit);
                const hiddenCount = selectedTagData.length - limit;

                selectedTagData.forEach((item, index) => {
                    const removeButton = hasTasks ? '' : `<button type="button" class="btn-close btn-close-white ms-1" style="font-size:8px;" data-remove="${item.id}" title="Remove"></button>`;
                    const tagHtml = `
                        <span class="badge bg-primary d-inline-flex align-items-center gap-1 px-2 py-1 fs-6" data-id="${item.id}" style="cursor:default; max-width:300px; white-space:normal; text-align:left;">
                            <span style="font-size:11px; line-height:1.3;">${item.text}</span>
                            ${removeButton}
                        </span>`;
                    ...
                    $('#allTagsModalBody').append(tagHtml);
                });

                if (hiddenCount > 0) {
                    const $moreBtn = $(`
                        <span class="badge bg-secondary d-inline-flex align-items-center gap-1 px-2 py-1 fs-6" style="cursor:pointer;" onclick="new bootstrap.Modal(document.getElementById('allTagsModal')).show()">
                            <span style="font-size:11px; line-height:1.3;">+${hiddenCount} More...</span>
                        </span>`);
                    $tags.append($moreBtn);
                }
            }
            ...
            function addTag(id, text) {
                ...
                selectedIds.push(id);
                selectedTagData.push({id: id, text: text});
            ...
            function removeTag(id) {
                ...
                selectedTagData = selectedTagData.filter(x => x.id !== id);
            ...
            if (window._initialStockEntries && window._initialStockEntries.length > 0) {
                window._initialStockEntries.forEach(function (entry) {
                    ...
                    if (!selectedTagData.find(x => x.id === String(entry.id))) {
                        selectedTagData.push({id: String(entry.id), text: entry.text});
                    }
                });
                renderAllTags();
            }
...
;
        ...
        function renderItemDetailsTable(artData) {
            ...
            if (fabricIndex === 0) {
                $fabricTbody.append(`<tr class="no-data-row"><td colspan="5" class="text-center text-muted">No Fabric Items</td></tr>`);
            }
            if (accIndex === 0) {
                $accTbody.append(`<tr class="no-data-row"><td colspan="5" class="text-center text-muted">No Accessories Items</td></tr>`);
            }
            
            setupPagination('item-details-fabric-tbody', 10);
            setupPagination('item-details-accessories-tbody', 10);
        }

        function showTablePage(tbodyId, page, rowsPerPage, totalPages) {
            const $tbody = $('#' + tbodyId);
            const $rows = $tbody.find('tr:not(.no-data-row)');
            ...
            let $nav = $('#' + paginationId);
            $nav.data('current-page', page);
            
            const $ul = $nav.find('ul');
            $ul.empty();
            
            $ul.append(`<li class="page-item ${page === 1 ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${page - 1}">Previous</a></li>`);
            
            let startPage = Math.max(1, page - 2);
            let endPage = Math.min(totalPages, page + 2);
            if (startPage > 1) {
                $ul.append(`<li class="page-item"><a class="page-link" href="#" data-page="1">1</a></li>`);
                if (startPage > 2) $ul.append(`<li class="page-item disabled"><span class="page-link">...</span></li>`);
            }
            ...
            $ul.append(`<li class="page-item ${page === totalPages ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${page + 1}">Next</a></li>`);
        }

        function setupPagination(tbodyId, rowsPerPage) {
            const $tbody = $('#' + tbodyId);
            const $rows = $tbody.find('tr:not(.no-data-row)');
            ...
            const $nav = $(`<nav id="${paginationId}" class="mt-3 custom-pagination" data-target-tbody="${tbodyId}" data-total-pages="${totalPages}" data-rows-per-page="${rowsPerPage}" data-current-page="1"><ul class="pagination pagination-sm justify-content-end mb-0"></ul></nav>`);
            $tbody.closest('table').after($nav);
            
            showTablePage(tbodyId, 1, rowsPerPage, totalPages);
        }
        ...
        $(document).on('click', '.custom-pagination a.page-link', function(e) {
            ...
            const tableId = $this.closest('.custom-pagination').data('target-tbody');
            const totalPages = parseInt($this.closest('.custom-pagination').data('total-pages'));
            const rowsPerPage = parseInt($this.closest('.custom-pagination').data('rows-per-page'));
            
            if (!isNaN(p) && p >= 1 && p <= totalPages) {
                showTablePage(tableId, p, rowsPerPage, totalPages);
            }
        });
