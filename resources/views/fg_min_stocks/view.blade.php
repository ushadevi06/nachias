...
@section('title', 'Finished Goods Minimum Stocks - ' . env('WEBSITE_NAME'))
@section('content')
...
<script>
    $(function() {
        ...
        $(document).on('change', '.fg-min-stock-status-toggle', function() {
            let id = $(this).data('id');
            let status = $(this).is(':checked') ? 'Active' : 'Inactive';

            $.ajax({
                url: "{{ url('fg-min-stocks/status') }}/" + id,
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    status: status
