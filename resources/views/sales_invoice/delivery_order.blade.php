...
<body>
    @php
        ...
        foreach ($groupedItems as $groupName => $items) {
            ...
            foreach ($items as $item) {
                ...
                $desc = trim(
                    ($brandName ? $brandName . ' ' : '') . ($styleName ? $styleName . ' ' : '') . $sleeveDisplay,
                );
                if (empty($desc)) {
                    $desc = $item->sku ?? '-';
                }
    ...
    @foreach ($chunks as $pageIndex => $chunk)
        <table class="header-table">
            <tr>
                ...
                <td style="text-align: left; vertical-align: top;">
                    <div style="font-size: 16px; font-weight: normal; margin: 0; line-height: 1.2;">
                        {{ $setting->company_name ?? 'Nachias Fashion Private Limited' }}</div>
                    <div style="font-size: 12px; margin: 2px 0 0 0; line-height: 1.2; color: #000;">
                        {!! nl2br(
                            e(
                                $setting->address ??
                                    "272/2, Somu Nagar, Siringeri Nagar\n(Sarathambal Kovil Backside),\nByepass Road, Madurai - 625016",
                            ),
                        ) !!}
                    </div>
                    <div style="font-size: 12px; margin: 2px 0 0 0; line-height: 1.2; color: #000;">
            ...
            <tr>
                <td style="padding: 6px 8px;">
                    <table style="width: 100%; border: none; border-collapse: collapse; margin: 0; padding: 0;">
                        <tr>
                            <td
                                style="width: 50px; border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                M/S</td>
                            <td
                                style="width: 15px; border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                :</td>
                ...
                <td style="padding: 6px 8px;">
                    <table style="width: 100%; border: none; border-collapse: collapse; margin: 0; padding: 0;">
                        <tr>
                            <td
                                style="width: 75px; border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                PL No.</td>
                            <td
                                style="width: 15px; border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                :</td>
                        ...
                        <tr>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">Packed On
                            </td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">:</td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                {{ $invoice->inv_date->format('d/m/Y') }}</td>
                        </tr>
                        <tr>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">Order No.
                            </td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">:</td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                {{ $invoice->salesOrder->order_no ?? ($invoice->salesOrder->so_no ?? '-') }}</td>
                        </tr>
                        <tr>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">Total Qty
                            </td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">:</td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                {{ number_format($totalPcs, 0) }}</td>
                        </tr>
                        <tr>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">No.of Units
                            </td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">:</td>
                            <td style="border: none; padding: 2px 0; vertical-align: top; font-size: 13px;">
                                {{ $invoice->no_of_box ?: 1 }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
                ...
                @foreach ($chunk as $rowItem)
                    @if ($rowItem['type'] == 'group_header')
                        <tr>
                            @php
                                ...
                                if (
                                    is_null($invoice->delivery_show_fields) ||
                                    in_array('art_no', $invoice->delivery_show_fields)
                                ) {
                                    $totalCols++;
                                }
                                if (
                                    is_null($invoice->delivery_show_fields) ||
                                    in_array('mrp', $invoice->delivery_show_fields)
                                ) {
                                    $totalCols++;
                                }
                                if (
                                    is_null($invoice->delivery_show_fields) ||
                                    in_array('price', $invoice->delivery_show_fields)
                                ) {
                                    $totalCols++;
                                }
                            @endphp
                            <td colspan="{{ $totalCols }}" class="group-header-bar"
                                style="padding: 4px 6px; font-weight: bold; font-size: 14px; background-color: #60a5fa; color: #000; border: 1px solid #000;">
                                {{ $rowItem['name'] }}
                            </td>
                        </tr>
                    @elseif($rowItem['type'] == 'item')
                        ...
                        <tr>
                            ...
                            @endif
                            @if (is_null($invoice->delivery_show_fields) || in_array('mrp', $invoice->delivery_show_fields))
                                <td class="text-right" style="padding-right: 6px;">{{ number_format($row['mrp'], 2) }}
                                </td>
                            @endif
                            @if (is_null($invoice->delivery_show_fields) || in_array('price', $invoice->delivery_show_fields))
                                <td class="text-right" style="padding-right: 6px;">{{ number_format($row['rate'], 2) }}
                                </td>
                            @endif
                            @foreach ($allSizes as $size)
                        ...
                        <tr class="bold bg-light-blue">
                            ...
                            @foreach ($allSizes as $size)
                                <td class="text-center">
                                    {{ $rowItem['size_totals'][$size] > 0 ? number_format($rowItem['size_totals'][$size], 0) : '0' }}
                                </td>
                            @endforeach
        ...
        @if ($pageIndex == count($chunks) - 1)
            {{-- Use the same bottom-footer treatment as Open Delivery Order. --}}
            ...
            <div style="position: absolute; bottom: 0; left: 0; width: 100%;">
                <table style="width: 100%; border-collapse: collapse; page-break-inside: avoid;">
                    ...
                    <tr>
                        <td
                            style="border: 1px solid #000000; width: 50%; height: 80px; text-align: center; vertical-align: middle; font-size: 13px;">
                            Prepared By<br><br>
