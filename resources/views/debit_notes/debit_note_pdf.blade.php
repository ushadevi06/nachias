        ...
        <table width="100%" cellpadding="6" cellspacing="0" style="border-bottom:4px double #000; line-height:1.4;">
          <tr>
            <td>
              ...
              State: <strong>{{ $debitNote->supplier->state->state_name ?? '-' }} ({{ $debitNote->supplier->state->state_code ?? '' }})</strong> &nbsp;
              Tel: <strong>{{ $debitNote->supplier->mobile_no ?? '-' }}</strong><br>
              Email: <strong>{{ $debitNote->supplier->email ?? '-' }}</strong> | Website: <strong>{{ $debitNote->supplier->website_url ?? '-' }}</strong>
            </td>
          </tr>
        </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" class="content-table" style="border-collapse:collapse; table-layout:fixed; width:100%;">
      ...
      <tbody>
        <tr style="background-color:#f2f2f2;">
          ...
          <th style="border-bottom:1px solid #000; border-right:1px solid #000; font-size:10px; padding:6px 2px; text-align:center;">QUANTITY</th>
          <th style="border-bottom:1px solid #000; border-right:1px solid #000; font-size:10px; padding:6px 2px; text-align:center;">UOM</th>
          <th style="border-bottom:1px solid #000; border-right:1px solid #000; font-size:10px; padding:6px 2px; text-align:center;">RATE (Rs.)</th>
        ...
        <tr>
          ...
          <td style="padding:5px; text-align:center; font-size:11px; border-right:1px solid #000;">{{ $item->uom->uom_code ?? '-' }}</td>
        ...
        <tr style="border-top:1px solid #000; font-weight:bold;">
          ...
          <td colspan="2" style="text-align:right; padding:6px; border-top:1px solid #000;">{{ number_format($debitNote->sub_total, 2) }}</td>
        </tr>

        @if($debitNote->other_state == 'Y')
        <tr>
          <td colspan="4" style="border-right:1px solid #000; padding:4px; text-align:right;">IGST ({{ $debitNote->igst_percent }}%)</td>
          <td colspan="2" style="text-align:right; padding:4px;">{{ number_format($debitNote->tax_amount, 2) }}</td>
        </tr>
        @else
        <tr>
          <td colspan="4" style="border-right:1px solid #000; padding:4px; text-align:right;">CGST ({{ $debitNote->cgst_percent }}%)</td>
          <td colspan="2" style="text-align:right; padding:4px;">{{ number_format($debitNote->tax_amount / 2, 2) }}</td>
        </tr>
        <tr>
          <td colspan="4" style="border-right:1px solid #000; padding:4px; text-align:right;">SGST ({{ $debitNote->sgst_percent }}%)</td>
          <td colspan="2" style="text-align:right; padding:4px;">{{ number_format($debitNote->tax_amount / 2, 2) }}</td>
        </tr>
        @endif
        ...
        <tr>
          <td colspan="4" style="border-right:1px solid #000; padding:4px; text-align:right;">Round Off ({{ $debitNote->round_off_type }})</td>
          <td colspan="2" style="text-align:right; padding:4px;">{{ $debitNote->round_off_type == 'Less' ? '-' : '' }}{{ number_format($debitNote->round_off, 2) }}</td>
        </tr>
        @endif

        <tr style="background-color:#f2f2f2; font-weight:bold; border-top:1px solid #000; border-bottom:1px solid #000;">
          <td colspan="4" style="border-right:1px solid #000; padding:8px; text-align:right; font-size:12px;">Grand Total (Rs.)</td>
          <td colspan="2" style="text-align:right; padding:8px; font-size:12px;">{{ number_format($debitNote->grand_total, 2) }}</td>
        </tr>
        ...
        <!-- SIGNATURE SECTION -->
        <tr>
          <td colspan="3" style="border-right:1px solid #000; border-bottom:1px solid #000; padding:30px 10px 10px; vertical-align:bottom;">
